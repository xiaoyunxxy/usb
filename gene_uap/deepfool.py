...
def deepfool(image, net, num_classes, overshoot, max_iter, target_label=0):
    ...
    f_image = net(Variable(image[None, :, :, :], requires_grad=True)).data.to(device).numpy().flatten()
    I = f_image.argsort()[::-1]
    ...
    while k_i != target_label and loop_i < max_iter:
        ...
        # ---
        if x.shape == torch.Size([3,32,32]):
            x = x[None, :]
        # ---

        fs = net(x)
        k_i = np.argmax(fs.data.to(device).numpy().flatten())
